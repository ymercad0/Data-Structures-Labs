...
public class Lab09P2Wrapper {
    ...
    public static class BinaryTree<E> implements Tree<E> {
        ...
        public List<E> getLevelElements(int level) {
            ...
            List<E> results = new SinglyLinkedList<>();
            getRecLevelElements(root, level, results);
            return results;
        }

        /*RECURSIVE AUXILIARY METHOD*/
        private void getRecLevelElements(BinaryTreeNode<E> root, int level, List<E> elements) {
            helperGetRecLevelElements(root, level, 1, elements);
        }

        private void helperGetRecLevelElements(BinaryTreeNode<E> root, int level, int currLevel, List<E> elements) {
            if(root.leftChild != null){
                helperGetRecLevelElements(root.leftChild, level, currLevel+1, elements);
            }

            if(currLevel == level){ //root != null is redundant
                elements.add(root.getValue());
            }

            if(root.rightChild != null){
                helperGetRecLevelElements(root.rightChild, level, currLevel+1, elements);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        List<Integer> answ = tree.getLevelElements(4);

        for(int i = 0; i < answ.size(); i++){
            System.out.println("Index " + i + ": " + answ.get(i));
        }
