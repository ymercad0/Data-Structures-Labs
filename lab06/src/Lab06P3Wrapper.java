...
public class Lab06P3Wrapper {
    ...
    public static class DoublyLinkedList<E> implements List<E> {
        ...
        public void print(){
            for(Node node = header; node != trailer; node = node.next){
        ...
        @Override
        public void reverse() {
            Node temp = header; //swap heads, head becomes tail and tail becomes the head
            header = trailer;
            trailer = temp;
            Node current = header; //traverses

            while(current != trailer) {
                temp = current.next; //stores the next link in a temporary variable so that current can point to it
                current.next = current.prev; //reverse linking
                current.prev = temp; //using stored previous reference to reverse the other link
